...
-- This migration ensures atomic updates for "Save Money" and "Withdraw Money" actions
...
-- Purpose: Deduct from main transactions, add to savings, record history
...
BEGIN
  ...
  -- 3. Calculate current main system balance (Transactions)
  SELECT
  ...
  -- 4. Check sufficient balance in main account
  ...
  -- 5. Get or create savings balance
  ...
  -- B. Create cash-out transaction in main system (Deduct from Transactions)
  ...
  -- C. Record the savings transaction history
...
EXCEPTION
  WHEN OTHERS THEN
    -- Rollback happens automatically in Postgres functions
    RETURN jsonb_build_object(
      'success', false,
...
-- Purpose: Deduct from savings, add to main transactions, record history
...
BEGIN
  ...
  -- 4. Check sufficient savings balance
  ...
  -- B. Create cash-in transaction in main system (Add to Transactions)
  INSERT INTO public.transactions (
